...
export function StatCard({ label, value, hint, icon:Icon, tone='brand' }) {
  ...
  return <motion.div whileHover={{y:-2}} className="panel p-4 md:p-5"><div className="flex items-start justify-between"><div><div className="text-xs font-bold uppercase tracking-[.14em] text-slate-500">{label}</div><div className="mt-2 font-[Outfit] text-2xl font-extrabold tracking-tight text-slate-100">{value}</div>{hint&&<div className="mt-1 text-xs text-slate-500">{hint}</div>}</div><div className={`grid h-11 w-11 place-items-center rounded-xl ${tones[tone]}`}><Icon size={20}/></div></div></motion.div>
}

export function EmptyState({ icon:Icon=PackagePlus, title, text, action }) {
  return <div className="flex flex-col items-center justify-center py-16 text-center"><div className="grid h-14 w-14 place-items-center rounded-2xl bg-slate-800 text-slate-500"><Icon size={25}/></div><div className="mt-4 font-[Outfit] text-lg font-bold text-slate-200">{title}</div><div className="mt-1 max-w-sm text-sm text-slate-500">{text}</div>{action&&<button onClick={action.onClick} className="btn-primary mt-5">{action.label}</button>}</div>
}

export function Modal({ title, onClose, children, wide=false }) { return <AnimatePresence><motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-50 grid place-items-center bg-slate-950/75 p-3 backdrop-blur-sm"><motion.div initial={{y:20,scale:.98}} animate={{y:0,scale:1}} exit={{y:10,scale:.98}} className={`max-h-[94vh] w-full overflow-y-auto rounded-3xl border border-slate-800 bg-slate-900 shadow-2xl ${wide?'max-w-3xl':'max-w-lg'}`}><div className="sticky top-0 flex items-center justify-between border-b border-slate-800 bg-slate-900/95 px-5 py-4 backdrop-blur"><h2 className="font-[Outfit] text-xl font-bold text-slate-100">{title}</h2><button onClick={onClose} className="rounded-xl p-2 text-slate-500 hover:bg-slate-800"><X size={19}/></button></div><div className="p-5">{children}</div></motion.div></motion.div></AnimatePresence> }
